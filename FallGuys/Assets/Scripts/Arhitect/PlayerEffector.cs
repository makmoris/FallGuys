        ...
        switch (bonus.Type)
        {
            case BonusType.DisableWeapon:
                ...
                if (!isShieldActive)
                {
                    ...
                    // здесь через gameObject потому что это событие не только дл€ нотификации, но еще и дл€ отключени€ на всех 
                    // если сделать только »гроку, то боты не будут знать, что им нужно отключить пушку
