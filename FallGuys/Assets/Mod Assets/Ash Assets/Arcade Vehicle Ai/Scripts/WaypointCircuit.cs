...
namespace ArcadeVP
{
    public class WaypointCircuit : MonoBehaviour
    {
        ...
        public bool addPointsFromSpline;
    ...
    public class WaypointListDrawer : PropertyDrawer
    {
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            var addSplinePointsButtonRect = new Rect(x, y, inspectorWidth, lineHeight);
            if (GUI.Button(addSplinePointsButtonRect, "Add points from spline"))
            {
                var circuit = property.FindPropertyRelative("circuit").objectReferenceValue as WaypointCircuit;

                // delete old
                int numChildren = circuit.transform.childCount;
                if (numChildren != 0)
                {
                    for (int i = numChildren - 1; i >= 0; i--)
                    {
                        UnityEngine.Object.DestroyImmediate(circuit.transform.GetChild(i).gameObject, false);
                    }
                }

                // create new
                SplineComputer splineComputer = circuit.GetComponent<SplineComputer>();
                SplinePoint[] splinePoints = splineComputer.GetPoints(SplineComputer.Space.World);

                for (int i = 0; i < splinePoints.Length; i++)
                {
                    GameObject waypoint = new GameObject("Waypoint");
                    waypoint.transform.SetParent(circuit.transform);
