...
public class ArenaCarDriverAI : DriverAI
{
    ...
    private List<Transform> bonusBoxes = new();// дл€ дуэли, чтобы чередовал игрока с бонусами
    ...
    public void SetTargets(List<Transform> _targets)
    {
        ...
        {
            if (item.GetComponent<Animator>() != null)// пока так, потом, если понадобитс€, можно создать какой-нибудь скрипт и кинуть на бокс
            {
                bonusBoxes.Add(item);
            }
